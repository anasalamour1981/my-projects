...
class Hijri(object):
    ...
    def check_gdate(self, gdate):
        result = False
        is_datetime = False
        if isinstance(gdate, basestring):
    ...
    def format(self):
        try:
            if not self.is_datetime:
                return u'{:0>2d}-{:0>2d}-{}'.format(self.raw[0], self.raw[1], self.raw[2])
            else:
                return u'{:0>2d}-{:0>2d}-{} {}'.format(self.raw[2], self.raw[1], self.raw[0], self.gdate.strftime('%H:%M:%S'))
